...
const MainLayout = ({ children }: { children: React.ReactNode }) => {
  // Simulating a guest state for now. In a real app, this would come from an Auth provider.
      ...
      <main className="flex-1 flex flex-col">
        <header className="h-16 border-b border-slate-100 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md flex items-center justify-between px-8 sticky top-0 z-10">
          <div className="flex items-center gap-4">
            {!isLoggedIn && (
              <span className="text-xs font-bold text-amber-600 bg-amber-50 px-2 py-1 rounded-md uppercase tracking-wider">
                Guest Mode
              </span>
            )}
          </div>
